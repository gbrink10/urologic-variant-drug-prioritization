...
P(f'The framework scored {F["n_drug_hypotheses"]} drug-cancer hypotheses across '
  f'those seven contexts, and reports one further, unscored biomarker '
  f'observation. {F["n_previously_proposed"]} of the hypotheses recover a '
  f'therapy proposed independently by another group, which is the positive '
  f'control; {F["n_partially_novel"]} extend a therapy from the conventional '
  f'form of the disease to a variant; and {F["funnel"]["framework_novel"]} '
  f'had no prior proposal in the urologic literature. Criteria stated before they were applied place '
...
P(f'What we think earns your reviewers\u2019 time is not any single drug-cancer '
  ...
  f'collected rather than reuse summary statistics, and it cost us: two '
  f'candidates stopped being prioritized, including the one with the most '
  f'attractive translational package. Second, that refit exposed '
...
P(f'The manuscript fits JCO Clinical Cancer Informatics specifically because the '
  f'contribution is methodological. Every scored component is emitted by one '
  f'function from the deposited fitted tables, so the manuscript table and '
  f'the deposited table cannot diverge. The candidate-selection rule is '
  f'stated before it is applied, and every candidate outside the priority '
  f'tier is reported with each criterion it missed named. That discipline is '
  f'also why the score has three dimensions rather than four: auditing the '
  f'curated genomic value against the cohorts it named reproduced the curated '
  f'band in four of the fifteen rows with a cohort to check against, so we '
  f'removed it from the score and report it as disease context. It changed no '
  f'prioritized candidate, which the deposited sensitivity table shows row by '
  f'row. A sensitivity analysis also reports how far the ordering depends on '
  f'the scoring architecture rather than the biology: each of the three '
  f'prioritized hypotheses rests on a different dimension and none survives '
  f'every variant, and we state that plainly. Rows whose evidence is not '
  f're-derivable from deposited data are flagged individually rather than '
  f'left implicit.')
...
P(f'The limitations are in the manuscript rather than in a closing sentence. '
  ...
  f'the therapy that serves as that disease\u2019s positive control. The lead '
