...
P('DepMap. What it is: a project that has switched off each gene, one at a '
  ...
  'strongly. That is one of the reservations recorded against that candidate, '
...
P('How the four were used. They are checks, not a filter. A candidate '
  'entered the priority tier only if no source contradicted it and the '
  ...
  'because a check that looks for contradiction and finds none is worth '
  'stating, and because a reader is entitled to know it changed nothing. A '
  'source that could not evaluate a candidate counted as neither support nor '
  'contradiction, which matters because silence from a tumor-cell screen '
  'about a drug that acts on the immune system is not evidence against that '
  'drug. Every candidate against every source is shown in Figure 4 and '
